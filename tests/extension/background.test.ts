...
describe('isCodeText', () => {
  it('returns false for text shorter than 8 chars', () => {
    expect(isCodeText('hello')).toBe(false);
  });

  it('returns false for text longer than 12000 chars', () => {
    expect(isCodeText('a'.repeat(12001))).toBe(false);
  });

  it('returns true for valid code', () => {
    expect(isCodeText('function hello() { return "world"; }')).toBe(true);
  });

  it('returns true for multi-line code', () => {
    expect(isCodeText('function hello() {\n  return 1;\n}')).toBe(true);
  });

  it('returns false for plain prose', () => {
    expect(isCodeText('This is a sentence. It does not contain code.')).toBe(false);
  });

  it('returns true for HTML', () => {
    expect(isCodeText('<div class="test">content</div>')).toBe(true);
  });

  it('returns true for Python code', () => {
    expect(isCodeText('def hello():\n    print("world")')).toBe(true);
  });

  it('returns true for CSS', () => {
    expect(isCodeText('.button { color: red; }')).toBe(true);
  });

  it('returns false for common English text', () => {
    expect(isCodeText('The quick brown fox jumps over the lazy dog.')).toBe(false);
  });

  it('returns false for Italian prose with parentheses and code words', () => {
    expect(isCodeText('Il framework (React) è molto usato; supporta la funzione map() e filter() per gli array.')).toBe(false);
  });

  it('returns false for prose that mentions the function keyword', () => {
    expect(isCodeText('This is the main function that runs when you press the button; it returns true (if valid) and prints the result.')).toBe(false);
  });

  it('returns false for prose about React hooks', () => {
    expect(isCodeText('The useState hook lets you add state to function components.')).toBe(false);
  });

  it('returns false for prose about console.log', () => {
    expect(isCodeText('Use console.log() to debug your code.')).toBe(false);
  });

  it('returns true for SQL', () => {
    expect(isCodeText('SELECT name, age FROM users WHERE age > 18 ORDER BY name;')).toBe(true);
  });

  it('returns true for JSON', () => {
    expect(isCodeText('{"name": "John", "age": 30, "city": "New York"}')).toBe(true);
  });

  it('returns true for simple assignments', () => {
    expect(isCodeText('x = y + 1;')).toBe(true);
  });

  it('returns true for a print call', () => {
    expect(isCodeText("print('hello world')")).toBe(true);
  });

  it('returns true for a React component snippet', () => {
    expect(isCodeText('const [count, setCount] = useState(0);')).toBe(true);
  });
});
